...
(function ($, window) {
    ...
    function initWebUpload(item, options) {
        ...
        var uploaderStrdiv = '<div class="webuploader">' +
            ...
            '<div class="btns">' +
            '<div id="' + pickerid + '"  class="btn btn-primary radius">选择PDF文件</div>' +
        ...
        var webuploaderoptions = $.extend({
            ...
            // 文件接收服务端。
            server: applicationPath + '/WebUploader/PDFProcess',
        ...
        if (fileDataStr && opts.PostbackHold) {
            ...
            $.each(jsonData.fileList, function (index, fileData) {
                ...
                '<div class="itemname">' + fileData.name + '</div>' +
        ...
        uploader.on('fileQueued', function (file) {
            $list.append('<div id="' + file.id + '" class="item">' +
                '<div class="itemname">' + file.name + '</div>' +
        ...
        $list.on("click", ".del", function () {
            ...
            $.each(jsonData.fileList, function (index, item) {
                if (item && item.queueId === id) {
                    ...
                    $.post(applicationPath + '/WebUploader/Delete', { 'filePathName': deletefile.filePath }, function (returndata) {
                        $ele.parent().remove();
                    });
                    return;
                }
    ...
    initWebUpload.prototype.deleteAll = function () {
        ...
        jsonData = JSON.parse(fileDataStr);
        $.post(applicationPath + '/WebUploader/DeleteAll', { 'filelist': jsonData }, function (returndata) {
            self.$list.empty();
        });
    }
    ...
    $.fn.powerWebUpload = function (options) {
        ...
        $(ele).each(function (idnex, item) {
            ...
            if (typeof PowerJs != 'undefined') {
                PowerJs.Core.lazyLoad(applicationPath + "/lib/webuploader/webuploader.css", function () { }, 'css');
                PowerJs.Core.lazyLoad(applicationPath + "/lib/webuploader/webuploader.min.js", function () {
