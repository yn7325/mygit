...
UserSensor.InitGrid = function (jcdbh) {
    grid = $('#list_data').datagrid({
        ...
        rownumbers: true,  //行号
        columns: [[
          { field: 'sensorNum', title: '传感器编号', width: 50 },
          { field: 'terminalNum', title: '终端编号', width: 50 },
          { field: 'stationName', title: '灾害点名称', width: 50 },
          { field: 'terminalName', title: '监测点名称', width: 50 },   
          { field: 'sensorName', title: '仪器名称', width: 50 },
          { field: 'sensorType', title: '仪器类型', width: 50 },
          { field: 'sensorUnit', title: '监测单位', width: 50 },
          { field: 'sensorAccurate', title: '精度', width: 50 },
          { field: 'longitude', title: '经度', width: 50 },
          { field: 'latitude', title: '纬度', width: 50 },
          { field: 'upperLimits', title: '上限值', width: 50 },
          { field: 'lowerLimits', title: '下限值', width: 50 },
          { field: 'treenum', title: '资源导航标签', width: 50 },
...
function sensorNum_add() {
    layer_show("添加传感器", "../User/AddsensorNum.html", '900', '900');
}

function terminalNum_add() {
    layer_show("添加终端", "../User/AddterminalNum.html", '', '630');
}

function stationNum_add() {
    layer_show("添加站点", "../User/AddIndex.html", '', '630');
}
...
UserSensor.InitGridterminal = function(jcdbh){
    ...
    grid = $('#list_data').datagrid({
        ...
        columns: [[
          { field: 'terminalNum', title: '终端编号', width: 50 },
          { field: 'stationNum', title: '站点编号', width: 50 },
          { field: 'location', title: '终端位置', width: 50 },         
          { field: 'longitude', title: '经度', width: 50 },
          { field: 'latitude', title: '纬度', width: 50 },
          { field: 'description', title: '描述', width: 50 },      
          { field: 'treenum', title: '资源导航标签', width: 50 },
          { field: 'id', title: '操作', width: 50, align: 'center', formatter: formatOperterminal }
