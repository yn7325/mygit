...
public class RemoteChanneldao {
	public List<RemoteChannel> GetChannelData(String rid){
		List<RemoteChannel> datalist = new ArrayList<>();
		Connection conn = GetConnection.getmysqlConnection();
		String sql="SELECT * FROM term_para_csp_config JOIN term_para_csm_config ON term_para_csp_config.RID = term_para_csm_config.RID\r\n"
				+ "WHERE term_para_csp_config.RID=\""+rid+"\"";
		Statement stmt = null;
		ResultSet rs = null;
		try{
